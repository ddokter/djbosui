...
def add_action(model):
    ...
    """ return Create URL for given model class or name """
...
def create_url(model):
    ...
    try:
        return reverse("create_%s" % model_name)
    except NoReverseMatch:
        return reverse("create", kwargs={'model': model_name})


@register.simple_tag
def inline_create_url(model_name, parent, extra_args=""):

    return "%s%s" % (reverse("inline_create", kwargs={
        'parent_pk': parent.id,
        'parent_model': get_model_name(parent),
        'model': model_name}), extra_args)
